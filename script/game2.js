class Game2 {
    ...
    step(domElement) {
        ...
        for (let i = 0; i < this.field.length; i++) {
            ...
            if (flag == true && domElement == this.field[i] && this.field[i].innerHTML == '') {
                flag = false;
    ...
    botStep() {
        let empty = [];
        for (let i = 0; i < this.field.length; i++) {
            if (this.field[i].innerHTML == '') {
                empty.push(this.field[i]);

            }
        }
        if (empty.length) {
            let randomIndex = getRandomInt(0, empty.length - 1)
            empty[randomIndex].innerHTML = 'O';
            this.arr.push(empty[randomIndex])
        }
